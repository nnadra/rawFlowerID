...
const CardPopular = ({product}) => {
  ...
  return (
    <div className="p-3 mx-3 bg-[url('src/assets/image.png')] w-fit border-2 border-[#4E2A1E] rounded-2xl mb-5">
      ...
      {/* Text */}
      <div className='p-3'>
        <h1 className='text-3xl my-1 font-bold text-[#4E2A1E]'>{product.name}</h1>
        <p className='text-xl text-[#4E2A1E]'>{product.category?.name}</p>
        <div className='flex mt-3 justify-between items-center'>
            <p className='text-2xl text-[#4E2A1E]'>Rp {Number(product.original_price).toLocaleString('id-ID')}</p>
            <button type="button" className='bg-[#4E2A1E] p-3 text-white rounded-md font-bold'>Detail</button>
        </div>
      </div>
    </div>
  )
